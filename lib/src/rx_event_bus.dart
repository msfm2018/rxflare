...
class _EventTask {
  ...
  _EventTask({required this.module, required this.eventID, required this.data, required this.uuid, required this.priority, required this.parallel});
}
...
class RxEventBus {
  ...
  // 注册（修复了类型赋值错误）
  // ==========================
  static void on<T>({required String module, required int eventID, required EventCallback<T> callback, EventToken? token, bool sticky = false}) {
  ...
  static void notify<T>({
    ...
    final task = _EventTask(module: module, eventID: eventID, data: data, uuid: uuid, priority: priority, parallel: parallel);
